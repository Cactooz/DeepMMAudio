...
class Runner:
    ...
    def train_fn(
        ...
        x0, x1, xt, (clip_f, sync_f, text_f, depth_f) = self.fm.get_x0_xt_c(x1,
                                                                   t,
                                                                   Cs=[clip_f, sync_f, text_f, depth_f],
                                                                   generator=self.rng)
        ...
        depth_f[null_video] = self.network.module.empty_depth_feat

        samples = torch.rand(bs, device=x1.device, generator=self.rng)
        null_text = (samples < self.null_condition_probability)
        text_f[null_text] = self.network.module.empty_string_feat
        
        pred_v = self.network(xt, clip_f, sync_f, text_f, t, depth_f)
    ...
    def train_pass(self, data, it: int = 0):
        ...
        with torch.amp.autocast('cuda', enabled=self.use_amp, dtype=torch.bfloat16):
            ...
            depth_f = data['depth_features'].cuda(non_blocking=True)
            ...
            depth_f[~video_exist] = self.network.module.empty_depth_feat
            ...
            if it % self.log_extra_interval == 0:
                ...
                unmasked_depth_f = depth_f.clone()
            x1, loss, mean_loss, t = self.train_fn(clip_f, sync_f, text_f, depth_f, a_mean, a_std)
        ...
        with torch.amp.autocast('cuda', enabled=self.use_amp,
            ...
            try:
                if it % self.log_extra_interval == 0:
                    ...
                    depth_f = unmasked_depth_f[0:1]
                    conditions = self.network.module.preprocess_conditions(clip_f, sync_f, text_f, depth_f)
    ...
    def validation_pass(self, data, it: int = 0):
        ...
        with torch.amp.autocast('cuda', enabled=self.use_amp, dtype=torch.bfloat16):
            ...
            self.log.data_timer.end()
            loss, mean_loss, t = self.val_fn(clip_f.clone(), sync_f.clone(), text_f.clone(), depth_f.clone(), x1)
